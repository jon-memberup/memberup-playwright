...
test('Add Post as Admin', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'DEV Automation (Jon)' })).toBeVisible();await page.getByText('Community', { exact: true }).click();
  ...
  await page.getByRole('textbox', { name: 'Title' }).fill('New Post (Automated) ' + generateRandomString());
